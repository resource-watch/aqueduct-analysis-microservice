...
from aqueduct.middleware import (
    get_geo_by_hash,
    sanitize_parameters,
    is_microservice_or_admin,
)
from aqueduct.routes.api import error
from aqueduct.serializers import (
    serialize_response,
    serialize_response_cba,
    serialize_response_default,
...
def analyze(**kwargs):
    ...
    try:
        ...
        if nPoints > 500:
            return error(
                status=500,
                detail=f"Error: Row number should be less or equal to 500, provided: {nPoints}",
            )
        ...
        data, downloadUrl = CartoService.get_table(
            points,
            kwargs["sanitized_params"]["analysis_type"],
            kwargs["sanitized_params"]["wscheme"],
            kwargs["sanitized_params"]["month"],
            kwargs["sanitized_params"]["year"],
            kwargs["sanitized_params"]["change_type"],
            kwargs["sanitized_params"]["indicator"],
            kwargs["sanitized_params"]["scenario"],
            locations,
            input_address,
            match_address,
            ids,
...
@aqueduct_analysis_endpoints_v1.route(
    "/cba/expire-cache", strict_slashes=False, methods=["POST"]
)
@is_microservice_or_admin
def expire_cache():
...
@aqueduct_analysis_endpoints_v1.route(
    "/cba/widget/<widget_id>", strict_slashes=False, methods=["GET"]
)
@sanitize_parameters
@validate_params_cba
        ...
        if "format" in request.args and request.args.get("format") == "json":
            return (
                jsonify(
                    serialize_response_cba(
                        json.loads(
                            json.dumps(output.get_widget(widget_id), ignore_nan=True)
                        )
                    )
                ),
                200,
                {
                    ...
                    "filename": "{0}.json".format(widget_id),
                },
            )
        elif "format" in request.args and request.args.get("format") == "csv":
            return (
                pd.DataFrame(output.get_widget(widget_id)["data"]).to_csv(),
                200,
                {
                    "Content-Type": "text/csv",
                    "Content-Disposition": "attachment",
                    "filename": "{0}.csv".format(widget_id),
                },
            )
        else:
            return (
...
@aqueduct_analysis_endpoints_v1.route(
    "/cba/default", strict_slashes=False, methods=["GET"]
)
@sanitize_parameters
@validate_params_cba_def
...
@aqueduct_analysis_endpoints_v1.route(
    "/risk/widget/<widget_id>", strict_slashes=False, methods=["GET"]
)
@sanitize_parameters
@validate_params_risk
...
@aqueduct_analysis_endpoints_v1.route(
    "/food-supply-chain/<user_indicator>/<threshold>",
        ...
        if file and allowed_file(file.filename):
            ...
            if destination.endswith("b64"):
                ...
                return (
                    jsonify(
                        {
                            "msg": "You have to base64 encode your file and add a b64 extension."
                        }
                    ),
                    500,
                    {},
            ...
            logging.info(
                '[ROUTER]: Analyzing supply chain. user_indicator="{}" threshold="{}" '.format(
                    user_indicator, threshold
                )
            )
            ...
            return error(
                status=500,
                detail="Cannot save file {}. Check file extension".format(
                    file.filename
                ),
            )
    except AttributeError as e:
...
@aqueduct_analysis_endpoints_v1.route(
    "/food-supply-chain/<job_token>", strict_slashes=False, methods=["GET"]
)
@sanitize_parameters
# @validate_params_cba_def
def get_supply_chain_analysis_result(job_token, **kwargs):
    try:
        logging.info(
            '[ROUTER]: Getting status/results. job_token="{}"'.format(job_token)
        )
