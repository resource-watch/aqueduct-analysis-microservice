...
class CBAService(object):
    def __init__(self, user_selections):
        ...
        self.cba_types = ['pop_costs', "gdp_costs", "urb_benefits", "pop_benefits", "gdp_benefits", "prot_present", "prot_future"]
    ...
    def compute_costs(self,m, input_total_cost, exposure):
        ...
        build_years = np.arange(self.build_start_end[1]-self.build_start_end[0])+1. # list of build years (starting at 1)
        cum_costs_present = np.cumsum(float(input_total_cost)*np.ones(len(build_years))/len(build_years))  # cumulative building costs until end of building period
        maintenance = cum_costs_present*self.om_costs  # maintenance costs
        costs_pa_present = np.diff(np.append(0., cum_costs_present)) + maintenance  # compute annual costs without discount rate until end of building
        costs_horizon_present = np.zeros(len(time_series))
        costs_horizon_present[self.build_start_end[0] - self.year_range[0]: self.build_start_end[1] - self.year_range[0]] = costs_pa_present  # insert costs until end of build
        costs_horizon_present[self.build_start_end[1] - self.year_range[0]:] = maintenance[-1]  # add maintenance until horizon
    ...
    def compute_benefits(self, model, annual_risk_pres, annual_risk_fut, annual_pop_pres, annual_pop_fut, annual_gdp_pres, annual_gdp_fut, annual_prot_pres, annual_prot_fut):
        ...
        relative_benefit = np.maximum(np.minimum(self.extrap1d(interp1d(list(self.benefit_increase), [0., 1.]))(self.time_series), 1.), 0.)
        ...
        urb_benefits_discounted = urb_benefits / ((1 + self.discount_rate) ** (self.year_array))  # add the annual discount rate
    ...
    def average_prot(self, m, year, risk_data_input):
        ...
        # PULL ONLY CURRENT DATA
            cols =[col for col in sqlalchemy.Table(self.df_urb_agg, self.metadata).columns.keys() if (clm.lower() in col) and (mdl.lower() in col) and (sco.lower() in col) and (year in col)]
            ...
            impact_present = pd.read_sql_query("SELECT {0} FROM {1} where id ='{2}'".format(', '.join(cols), self.df_urb_agg, self.geogunit_name), self.engine).iloc[0]
            check = 1e25
    ...
    def risk_evolution(self, impact_cc, impact_urb, impact_pop, impact_gdp, prot, prot_idx):
        ...
        for year, imp_cc, imp_urb, imp_pop, imp_gdp in zip(self.years, impact_cc, impact_urb, impact_pop, impact_gdp):
            prot_trans = self.compute_rp_change(self.rps, impact_cc[prot_idx], imp_cc, prot, min_rp=2, max_rp=1000) # i.e. RP_zero
        ...
        # print prot_levels
        risk_func = interp1d(self.years, risk_prot, kind = 'linear', bounds_error = False, fill_value = 'extrapolate') #define interpolation function/relationship
        pop_func = interp1d(self.years, pop_impact, kind = 'linear', bounds_error = False, fill_value = 'extrapolate')  # define interpolation function/relationship
        gdp_func = interp1d(self.years, gdp_impact, kind = 'linear', bounds_error = False, fill_value = 'extrapolate')  # define interpolation function/relationship
    ...
    def precalc_present_benefits(self, model):
        ...
        urb_imp = self.filt_risk[[col for col in self.filt_risk.columns if ("urban_damage" in col) and (self.scen_abb.lower() in col)
                              and (model.lower() in col) and ("tot" in col)]].sum(axis=0)
        pop_imp = self.filt_risk[[col for col in self.filt_risk.columns if ("popexp" in col) and (self.scen_abb.lower() in col)
                              and (model.lower() in col) and ("tot" in col)]].sum(axis=0)
        gdp_imp = self.filt_risk[[col for col in self.filt_risk.columns if ("gdpexp" in col) and (self.scen_abb.lower() in col)
                              and (model.lower() in col) and ("tot" in col)]].sum(axis=0)
        prot_imp = self.df_prot.ix[self.geogunit_name, [col for col in self.df_prot.columns if ("urban_damage" in col) and (self.scen_abb.lower() in col)
                              and ("prot_avg" in col)]]

        risk_func = interp1d(self.years, urb_imp, kind = 'linear', bounds_error = False, fill_value = 'extrapolate') #define interpolation function/relationship
        pop_func = interp1d(self.years, pop_imp, kind = 'linear', bounds_error = False, fill_value = 'extrapolate')  # define interpolation function/relationship
        gdp_func = interp1d(self.years, gdp_imp, kind = 'linear', bounds_error = False, fill_value = 'extrapolate')  # define interpolation function/relationship
        prot_func = interp1d(self.years, prot_imp, kind='linear', bounds_error=False, fill_value='extrapolate')  # define interpolation function/relationship
...
class CBAEndService(object):
    ...
    def widget_impl_cost(self):
        ...
        fOutput['value'] = (fOutput['gdp_costs_avg'] * (1+self.data['meta']['discount']) * (fOutput['year'] - minY )) / (1 + om_costs)
